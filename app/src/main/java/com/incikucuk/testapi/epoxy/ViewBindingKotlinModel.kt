package com.incikucuk.testapi.epoxy
...
abstract class ViewBindingKotlinModel<T : ViewBinding>(
    ...
    override fun bind(view: View) {
        var binding = view.getTag(com.airbnb.viewmodeladapter.R.id.epoxy_recycler_view_child_initial_size_id) as? T   //R.id.epoxy_viewbinding)
        if (binding == null) {
            binding = bindingMethod.invoke(null, view) as T
            view.setTag(com.airbnb.viewmodeladapter.R.id.epoxy_recycler_view_child_initial_size_id, binding)
        }
        binding.bind()
    }
